...
def _scenario(scenario_id: str, mode: str = "suite_as_client") -> wire_target.LiveWireScenario:
    return wire_target.LiveWireScenario(
        scenario_id,
        mode,
        wire_target.LiveWireTransport("tcp", "tcp://127.0.0.1:19091"),
    )
...
def test_run_server_scenarios_coordinates_threads_and_reports_errors(
    ...
    def complete(_scenario, ready_event, **_kwargs) -> None:
        ready_event.set()

    monkeypatch.setattr(wire_target, "_run_server_scenario", complete)
    ready = tmp_path / "ready"
    wire_target._run_server_scenarios(scenarios, ready_path=ready, timeout_seconds=1.0)
    assert ready.exists()

    def fail(_scenario, ready_event, **_kwargs) -> None:
        del ready_event
        raise ValueError("broken target")

    monkeypatch.setattr(wire_target, "_run_server_scenario", fail)
    with pytest.raises(RuntimeError, match="live wire scenario failed") as captured:
...
def test_run_server_scenario_dispatches_typed_handlers(
    scenario_id: str,
    handler_name: str,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    called: list[object] = []
    for name in ("_handle_cancel_server", "_handle_priority_server", "_handle_cache_server"):
        monkeypatch.setattr(wire_target, name, lambda value, **kwargs: called.append(value))
    ready = threading.Event()
    wire_target._run_server_scenario(_scenario(scenario_id), ready_event=ready, timeout_seconds=1.0)
    assert ready.is_set()
    assert called == [session]

    with pytest.raises(ValueError, match="unsupported live server scenario"):
        wire_target._run_server_scenario(_scenario("unknown"), ready_event=threading.Event(), timeout_seconds=1.0)
...
def test_server_handlers_exchange_typed_control_frames(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(wire_target, "_await_runtime_frames", await_frames)
    monkeypatch.setattr(wire_target, "_await_peer_close", lambda value, **kwargs: closed.append(value))
