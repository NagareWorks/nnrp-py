...
def _write_staging_wheel(path: Path) -> Path:
    with zipfile.ZipFile(path, "w") as archive:
        ...
        archive.writestr(
            "nnrp_py-1.0.0rc4.post9.dist-info/WHEEL",
            b"Wheel-Version: 1.0\nGenerator: test\nRoot-Is-Purelib: true\nTag: py3-none-any\n",
        )
        archive.writestr("nnrp_py-1.0.0rc4.post9.dist-info/METADATA", b"Name: nnrp-py\n")
        archive.writestr("nnrp_py-1.0.0rc4.post9.dist-info/RECORD", b"")
    return path


def test_build_native_wheels_splits_artifacts_and_retags_platforms(tmp_path: Path) -> None:
    source = _write_staging_wheel(tmp_path / "nnrp_py-1.0.0rc4.post9-py3-none-any.whl")

    built = build_native_wheels(source, tmp_path / "dist")

    assert [wheel.name for wheel in built] == [
        "nnrp_py-1.0.0rc4.post9-py3-none-ios_13_0_arm64_iphonesimulator.whl",
        "nnrp_py-1.0.0rc4.post9-py3-none-manylinux_2_28_x86_64.whl",
        "nnrp_py-1.0.0rc4.post9-py3-none-win_amd64.whl",
    ]
    with zipfile.ZipFile(built[0]) as archive:
        ios_names = set(archive.namelist())
    with zipfile.ZipFile(built[1]) as archive:
        names = set(archive.namelist())
        metadata = archive.read("nnrp_py-1.0.0rc4.post9.dist-info/WHEEL").decode()
    ...
    assert "nnrp_py-1.0.0rc4.post9.dist-info/RECORD" in names
...
def test_build_native_wheels_rejects_missing_native_payload(tmp_path: Path) -> None:
    wheel = tmp_path / "nnrp_py-1.0.0rc4.post9-py3-none-any.whl"
    with zipfile.ZipFile(wheel, "w") as archive:
        archive.writestr("nnrp/__init__.py", b"")
        archive.writestr("nnrp_py-1.0.0rc4.post9.dist-info/WHEEL", b"Tag: py3-none-any\n")
        archive.writestr("nnrp_py-1.0.0rc4.post9.dist-info/RECORD", b"")
...
def test_build_native_wheels_rejects_invalid_wheel_shapes(tmp_path: Path) -> None:
    no_dist_info = tmp_path / "nnrp_py-1.0.0rc4.post9-py3-none-any.whl"
