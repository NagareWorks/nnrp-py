...
- [ ] Replace preview2 single-session helper call sites with the preview3 connection/session model in place.
  - [ ] Inventory public client helper call sites that still assume one session per connection.
  - [ ] Redirect client bootstrap examples/tests to `connect_native_client_connection` where packet-level transport smoke is not required.
  - [ ] Redirect submit/result examples/tests to native session operations where raw packet coverage is not being tested.
  - [ ] Move old single-session transport helpers to tooling/smoke docs if they remain useful for adapter bring-up.
  - [ ] Update package exports only after replacement call sites are green.
